...
import DOMSplatter from "./DOMSplatter";
  ...
  return (
    <>
      ...
      <motion.div
        ...
        onClick={(e) => {
          // Emit a splat dynamically where the user clicks using screen-space coordinates
          const colors = ["#F97316", "#3B82F6", "#10B981", "#EAB308", "#EC4899"];
          const randomColor = colors[Math.floor(Math.random() * colors.length)];
          window.dispatchEvent(
            new CustomEvent("emit-splat", {
              detail: { color: randomColor, clientX: e.clientX, clientY: e.clientY },
            }),
          );
        }}
      >
      ...
      {!showFinal &&
        PHRASES.map((phrase, i) => (
          <motion.h1
            ...
            onAnimationStart={() => {
              // Delay the splat until opacity goes up slightly during this phrase's animation
              setTimeout(() => {
                window.dispatchEvent(
                  new CustomEvent("emit-splat", {
                    detail: { color: phrase.color },
                  }),
                );
              }, 300);
            }}
        ...
        >
          ...
          <motion.img
            ...
            onError={(e) => {
              // Failsafe fallback
              e.currentTarget.src =
                "https://drive.google.com/uc?export=view&id=1nDOUxeJHCAlH9YkKUs_P7-L08z6pVXU4";
            }}
          ...
          >
            ...
            <h1 className="font-serif text-3xl md:text-5xl lg:text-7xl leading-none text-slate-800 text-shadow-sm mb-4 drop-shadow-2xl mix-blend-multiply pointer-events-none">
              Resting Art
            </h1>
